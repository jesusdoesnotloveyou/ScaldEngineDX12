...
public:
    static constexpr inline D3D12_INPUT_LAYOUT_DESC InputLayout = {InputElements, InputElementCount};
};
...
struct VertexPositionNormalTangentUV
{
    ...
    VertexPositionNormalTangentUV(float px, float py, float pz, float nx, float ny, float nz, float tx, float ty, float tz, float u, float v)
        : position(px, py, pz),
          normal(nx, ny, nz),
          tangent(tx, ty, tz),
          texCoord(u, v)
